...
:: --- CONFIGURATION ---
:: Set this to 1 ONLY if you have an RTX 50-series card but auto-detection fails.
:: Default is 0 (Auto-detect).
set "MANUAL_50SERIES_OVERRIDE=0"
...
:: ==========================================================
:: ROBUST RTX 50-SERIES DETECTION LOGIC
:: ==========================================================
set "IS_RTX50=0"

:: 1. Check for Manual Override (set at top of script)
if "%MANUAL_50SERIES_OVERRIDE%"=="1" (
    echo Manual override enabled: Forcing RTX 50-series mode.
    set "IS_RTX50=1"
)

:: 2. Check for specific model numbers (More robust than regex)
:: Space separated strings in findstr act as OR
echo !GPU_NAME! | findstr /i "5090 5080 5070 5060 5050" >nul
if not errorlevel 1 set "IS_RTX50=1"

:: 3. Check for Blackwell architecture name
echo !GPU_NAME! | findstr /i "Blackwell" >nul
if not errorlevel 1 set "IS_RTX50=1"
...
if "!IS_RTX50!"=="1" (
    ...
    echo RTX 50xx series detected - using PyTorch 2.7.0 / CUDA 12.8
) else (
    set PYTORCH_VERSION=2.6.0
    set CUDA_VERSION=cu124
    echo RTX 40xx or older detected - using PyTorch 2.6.0 / CUDA 12.4
)
:: ==========================================================
